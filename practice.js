...
callback(false)   // "<" just use "array.length", if "<=" use "array.length - 1"
}
// Do not edit the code below.
...
function each(namesArray, callback) {
  for(let i = 0; i <= namesArray.length - 1; i++){
    callback(namesArray[0], i)
  }
}
//REVISIT
/*for each(namesArray, callback) {
...
function getUserById(users, id, callback) {
  for(let i = 0; i < users.length; i++) {
    if(id === users[i].id) {
      callback(users[i]) // remember "i" is an iteration. It's a variable
    }
  }
}
// Do not edit the code below.
